...
**Version: v1.2.3.** GitHub: [Vthree/rwf-watcher](https://github.com/Vthree/rwf-watcher)
...
| 新 best | 只報三家裡的**世界領先**尾王 `%`（階段更後，或同階段剩餘更低）。落後的個人 best 不報。用 live `progress_display` 顯示。**新階段（含隱藏 P4）會報**；同一階段必須 API `bestPercent` 也變好才報（避免當下這拉誤報） |
